...
// • Multi-path support (arrays of waypoints)
// • Independent damage system + XP/Gold rewards
// • Cached sprites for smoother animation
// ============================================================
...
async function loadAndCache(src, targetSize = 128) {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      cx.drawImage(img, 0, 0, img.width, img.height, 0, 0, targetSize, targetSize);
      resolve(c);
    };
  });
}
...
async function loadWorgSprites() {
  worgSprites = {
    ...
    run: {
      left: [await loadAndCache("./assets/images/sprites/worg/worg_A1.png"), await loadAndCache("./assets/images/sprites/worg/worg_A2.png")],
      right:[await loadAndCache("./assets/images/sprites/worg/worg_D1.png"), await loadAndCache("./assets/images/sprites/worg/worg_D2.png")],
      up:   [await loadAndCache("./assets/images/sprites/worg/worg_W1.png"), await loadAndCache("./assets/images/sprites/worg/worg_W2.png")],
      down: [await loadAndCache("./assets/images/sprites/worg/worg_S1.png"), await loadAndCache("./assets/images/sprites/worg/worg_S2.png")],
    },
...
export async function initWorg(paths) {
  if (Array.isArray(paths)) {
    allPaths = paths.length > 0 && Array.isArray(paths[0]) ? paths : [paths];
  } else {
    allPaths = [];
  }
  worgList = [];
...
export function drawWorg(ctx) {
  ...
  for (const w of worgList) {
    ...
    ctx.fillStyle = "#000";
    ctx.beginPath();
    ctx.ellipse(w.x, w.y + size * 0.45, size * 0.32, size * 0.13, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.globalAlpha = 1;
    ...
    if (w.slowTimer > 0 && w.alive) {
      ctx.save();
      const pulse = 0.85 + Math.sin(performance.now() / 200) * 0.12;
      ctx.globalCompositeOperation = "screen";
